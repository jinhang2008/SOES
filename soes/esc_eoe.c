...
int EOE_get_mac(uint8_t port, uint8_t mac[])
{
   ...
   {
      if(nic_ports[port].mac_set)
      {
         memcpy(mac, nic_ports[port].mac.addr , sizeof(nic_ports[port].mac));
         nic_ports[port].mac_set = 1;
         ret = 0;
      }
   }
   return ret;
...
int EOE_ecat_set_mac(uint8_t port, uint8_t mac[])
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      memcpy(nic_ports[port].mac.addr, mac , sizeof(nic_ports[port].mac));
      ret = 0;
   }
   return ret;
}
...
int EOE_ecat_get_ip(uint8_t port, uint32_t * ip)
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      if(nic_ports[port].ip_set)
      {
         *ip = EOE_NTOHL(nic_ports[port].ip.addr);
         ret = 0;
      }
   }
   return ret;
...
int EOE_ecat_set_ip(uint8_t port, uint32_t  ip)
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      nic_ports[port].ip.addr = EOE_HTONL(ip);
      nic_ports[port].ip_set = 1;
      ret = 0;
   }
   return ret;
}
...
int EOE_ecat_get_subnet(uint8_t port, uint32_t * subnet)
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      if(nic_ports[port].subnet_set)
      {
         *subnet = EOE_NTOHL(nic_ports[port].subnet.addr);
         ret = 0;
      }
   }
   return ret;
...
int EOE_ecat_set_subnet(uint8_t port, uint32_t subnet)
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      nic_ports[port].subnet.addr = EOE_HTONL(subnet);
      nic_ports[port].subnet_set = 1;
      ret = 0;
   }
   return ret;
}
...
int EOE_ecat_get_gateway(uint8_t port, uint32_t * default_gateway)
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      if(nic_ports[port].default_gateway_set)
      {
         *default_gateway = EOE_NTOHL(nic_ports[port].default_gateway.addr);
         ret = 0;
      }
   }
   return ret;
...
int EOE_ecat_set_gateway(uint8_t port, uint32_t default_gateway)
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      nic_ports[port].default_gateway.addr = EOE_HTONL(default_gateway);
      nic_ports[port].default_gateway_set = 1;
      ret = 0;
   }
   return ret;
}
...
int EOE_ecat_get_dns_ip(uint8_t port, uint32_t * dns_ip)
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      if(nic_ports[port].dns_ip_set)
      {
         *dns_ip = EOE_NTOHL(nic_ports[port].dns_ip.addr);
         ret = 0;
      }
   }
   return ret;
...
int EOE_ecat_set_dns_ip(uint8_t port, uint32_t dns_ip)
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      nic_ports[port].dns_ip.addr = EOE_HTONL(dns_ip);
      nic_ports[port].dns_ip_set = 1;
      ret = 0;
   }
   return ret;
}
...
int EOE_ecat_get_dns_name(uint8_t port, char * dns_name)
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      if(nic_ports[port].dns_name_set)
      {
         memcpy(dns_name,
               nic_ports[port].dns_name,
               sizeof(nic_ports[port].dns_name));
         ret = 0;
      }
   }
   return ret;
...
int EOE_ecat_set_dns_name(uint8_t port, char * dns_name)
{
   ...
   if(port < EOE_NUMBER_OF_PORTS)
   {
      memcpy(nic_ports[port].dns_name,
            dns_name,
            sizeof(nic_ports[port].dns_name));
      nic_ports[port].dns_name_set = 1;
      ret = 0;
   }
   return ret;
}
...
static void EOE_get_ip (void)
{
   ...
   port = ((EOE_HDR_FRAME_PORT & frameinfo1) >> 4);
   ...
   if (mbxhandle)
   {
      ...
      if(nic_ports[port].ip_set)
      {
         ...
         EOE_ip_uint32_to_byte(&nic_ports[port].ip,
      ...
      if(nic_ports[port].subnet_set)
      {
         ...
         EOE_ip_uint32_to_byte(&nic_ports[port].subnet,
               &eoembx->data[data_offset]);
      }
      /* Add size of uint32 IP address */
      data_offset += 4;
      ...
      if(nic_ports[port].default_gateway_set)
      {
         ...
         EOE_ip_uint32_to_byte(&nic_ports[port].default_gateway,
               &eoembx->data[data_offset]);
      }
      /* Add size of uint32 IP address */
      data_offset += 4;
      ...
      if(nic_ports[port].dns_ip_set)
      {
         ...
         EOE_ip_uint32_to_byte(&nic_ports[port].dns_ip,
               &eoembx->data[data_offset]);
      }
      /* Add size of uint32 IP address */
      data_offset += 4;
...
static void EOE_set_ip (void)
{
   ...
   {
      memcpy(&nic_ports[port].mac.addr,
            &eoembx->data[data_offset],
            EOE_ETHADDR_LENGTH);
      nic_ports[port].mac_set = 1;
   }
   ...
   {
      EOE_ip_byte_to_uint32(&eoembx->data[data_offset],
            &nic_ports[port].default_gateway);
      nic_ports[port].default_gateway_set = 1;
   }
   /* Add size of uint32 IP address */
   data_offset += 4;
   ...
   if(flags & EOE_PARAM_DNS_NAME_INCLUDE)
   {
      ...
      memcpy(nic_ports[port].dns_name, &eoembx->data[data_offset], dns_len);
      nic_ports[port].dns_name_set = 1;
   }
   data_offset += EOE_DNS_NAME_LENGTH;
